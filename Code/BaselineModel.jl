...
function ChoiceProb(pars,Y,price,nk,age,CV,WR)
	pWork = zeros(2) #<- work choice probabilities
	pF = zeros(2) #<- probability of formal care given work and welfare choice
	vA = zeros(2) #<- value from participation choice
	for p=0:1
		if age<=17 #<- assume quarterly
			...
			uF = (pars.αc+pars.Γ[age+1])*log(Y[nk,1+p,2]+(112-30)*pars.wq-price) + pars.αF - gF
			uN = (pars.αc+pars.Γ[age+1])*log(Y[nk,1+p,2]+(112-30)*pars.wq) - gN
			pF[p+1] = 1/(1+exp((uN-uF)/pars.σC)) #<- probability of formal care
			vW1 = pars.σC*log(exp(uN/pars.σC)+exp(uF/pars.σC)) - pars.αH
			vW0 = (pars.αc+pars.Γ[age+1])*log(Y[nk,1+p,1] + 112*pars.wq) - pars.αWR*WR*p
			pWork[p+1] = 1/(1+exp((vW0-vW1)/pars.σH))
			vA[p+1] = pars.σH*log(exp(vW0/pars.σH)+exp(vW1/pars.σH)) + pars.β*CV[p+1] - pars.αA*p
		else
			vW1 = pars.αc*log(Y[nk,1+p,2]+(112-30)*pars.wq) - pars.αH
			vW0 = pars.αc*log(Y[nk,1+p,1]+(112)*pars.wq)
			pWork[p+1] = 1/(1+exp((vW0-vW1)/pars.σH))
			vA[p+1] = pars.σH*log(exp(vW0/pars.σH)+exp(vW1/pars.σH)) + pars.β*CV[p+1] - pars.αA*p
		end
	end
...
# this function assumes we're working with quarterly data
function GetStaticProbs(pars,Y,price,WR,T,NK)
	pWork = zeros(T,NK,17,2)
	pF = zeros(T,NK,17,2)
	pA = zeros(T,NK,17)
	...
	for t=1:T
		for k=1:NK
			for a0 = 0:16
				a = a0+t-1
				pA[t,k,a0+1],pWork[t,k,a0+1,:],pF[t,k,a0+1,:],V = ChoiceProb(pars,Y[t,:,:],price,k,a,CV,WR)
			end
		end
	end
	return pA,pWork,pF
...
function GetMeanCare(year,a0,a1,pA,pWork,π0)
	NK = size(pA)[2]
	denom = 0
	numerator = 0
	for a=a0:a1
