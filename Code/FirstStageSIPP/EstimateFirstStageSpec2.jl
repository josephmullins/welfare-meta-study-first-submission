...
x0 = [zeros(NS);zeros(NS);40*ones(NS);w0;
    [3.,1.,10.,-1.,0.,0.,0.,0.]]
...
lb = [[-Inf*ones(NS);-Inf*ones(NS);zeros(NS);4*30*ones(NS)];
    [0.,0.,0.,-10.,-.1,-0.1]; -Inf*ones(2)]

ub = [Inf*ones(NS*4);[Inf,Inf,Inf,0.3,0.3,0.1]; Inf*ones(2)]
...
Γ = exp.(pars.Γ[1] .+ pars.Γ[2]*(0:18) .+ pars.Γ[3]*(0:18).^2)
writedlm("Gamma_est",Γ)
writedlm("gF",pars.gF)
